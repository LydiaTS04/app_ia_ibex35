...
with st.container():
    ...
    with c1: capital = st.number_input('Capital Total (€)', value=10000, step=1000)
    with c2: risk_pct = st.number_input('Riesgo Máximo Permitido (%)', value=1.5, step=0.1)
    with c3: entry_price = st.number_input('Precio Entrada (Pts)', value=float(live_last), step=10.0)
    with c4: stop_loss = st.number_input('Stop Loss (Pts)', value=float(live_last*0.98), step=10.0)
    ...
    if entry_price > 0 and stop_loss > 0 and entry_price != stop_loss:
        ...
        <div style='display:flex;gap:10px;margin-top:15px;flex-wrap:wrap;'>
           <div class='mpill' style='flex:1;min-width:140px;'><div class='lbl'>Títulos a Comprar</div><div class='val' style='color:#00D4FF;'>{int(position_size)}</div></div>
           <div class='mpill' style='flex:1;min-width:140px;'><div class='lbl'>Inversión Teórica</div><div class='val' style='color:#00FF88;'>{total_invest:,.2f} €</div></div>
           <div class='mpill' style='flex:1;min-width:140px;'><div class='lbl'>Pérdida (Stop Limit)</div><div class='val' style='color:#FF3366;'>{risk_amount:,.2f} €</div></div>
        </div>
        """, unsafe_allow_html=True)
    else:
